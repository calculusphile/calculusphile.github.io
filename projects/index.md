---
layout: default
title: Projects
---

# Projects

Welcome to the projects index. Explore the following projects:

<div class="projects-list" style="display:flex;flex-wrap:wrap;gap:2rem 2.5rem;justify-content:center;">
  <div class="project-card" style="min-width:240px;max-width:320px;background:#f8fafc;border-radius:10px;box-shadow:0 2px 8px #e2e8f0;padding:1.2rem 1rem;text-align:center;">
    <span style="font-size:2rem;">🤖</span>
    <h3 style="margin:0.7rem 0 0.3rem 0;font-size:1.18rem;font-weight:700;">
      <a href="{{ '/projects/face-recognition/' | relative_url }}" style="color:#1976d2;text-decoration:none;">Face Recognition</a>
    </h3>
    <div style="color:#607d8b;font-size:0.98rem;">A project on facial recognition using AI/ML techniques.</div>
  </div>
  <div class="project-card" style="min-width:240px;max-width:320px;background:#f8fafc;border-radius:10px;box-shadow:0 2px 8px #e2e8f0;padding:1.2rem 1rem;text-align:center;">
    <span style="font-size:2rem;">🛠️</span>
    <h3 style="margin:0.7rem 0 0.3rem 0;font-size:1.18rem;font-weight:700;">
      <a href="{{ '/projects/project-2/' | relative_url }}" style="color:#1976d2;text-decoration:none;">Project 2</a>
    </h3>
    <div style="color:#607d8b;font-size:0.98rem;">Description for Project 2 goes here.</div>
  </div>
</div>

<div style="text-align:center;margin-top:2.5rem;">
  <a href="{{ '/' | relative_url }}" style="color:#607d8b;">&larr; Back to home</a>
</div>
